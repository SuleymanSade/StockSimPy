...
class TestWildersSmoothing:
    def test_wilders_smoothing_valid_input(self):
        ...
        result_series = result[f"wilders_smoothing_{window}"]
...
class TestCalculateSMA:
    def test_calculate_sma_valid_input(self):
        ...
        result_series = result[f"sma_{window}"]
...
class TestCalculateRSI:
    ...
    def test_calculate_rsi_custom_window(self):
        ...
        result_series = result[f"rsi_{window}"]
        assert len(result_series) == len(data_series)
...
class TestCalculateEMA:
    def test_calculate_ema_basic(self):
        ...
        result_series = result[f"ema_{window}"]
...
class TestCalculateDEMA:
    def test_calculate_dema_basic(self):
        ...
        result_series = result[f"dema_{window}"]
...
class TestCalculateTEMA:
    def test_calculate_tema_basic(self):
        ...
        result_series = result[f"tema_{window}"]
...
class TestCalculateHMA:
    def test_calculate_hma_basic(self):
        ...
        ema_half_window = calculate_ema(data, window // 2)[f"ema_{window // 2}"]
        ema_full_window = calculate_ema(data, window)[f"ema_{window}"]
        raw_hma_component = (2 * ema_half_window) - ema_full_window

        sqrt_window = int(math.sqrt(window))
        if sqrt_window == 0:
            sqrt_window = 1
        expected = calculate_ema(raw_hma_component, sqrt_window)[f"ema_{sqrt_window}"]

        result = calculate_hma(data, window)
        result_series = result[f"hma_{window}"]
...
class TestCalculateTEMAMACD:
    ...
    def test_tema_macd_consistency_with_manual_calc(self):
        ...
        ema_fast = calculate_ema(data, fast)[f"ema_{fast}"]
        ema_slow = calculate_ema(data, slow)[f"ema_{slow}"]
